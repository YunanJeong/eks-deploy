...
이 프로젝트는 EKS 클러스터를 위한 독립적이고 격리된 환경을 구축합니다.

**신규 인프라 자동 생성**
기존에 생성된 네트워크 리소스를 재사용하지 않고, EKS 전용 VPC, 서브넷, 보안 그룹, 인터넷 게이트웨이 등을 완전히 새롭게 생성합니다. 이를 통해 다른 프로젝트와 독립된 깨끗한 환경에서 클러스터를 운영할 수 있습니다.

**네트워크 격리 및 보안**
워커 노드(EC2 인스턴스)는 외부에서 직접 접근할 수 없는 프라이빗 서브넷에 배치됩니다. 외부와의 통신은 NAT 게이트웨이를 통해서만 가능하며, 로드 밸런서(ALB/NLB)를 위한 퍼블릭 서브넷이 별도로 구성됩니다.

**공식 모듈 기반의 표준 구성**
AWS에서 검증한 공식 Terraform 모듈을 사용하여 클러스터 제어 평면(Control Plane)과 노드 간의 통신에 필요한 보안 그룹(Security Group) 규칙과 IAM 역할(Role)을 모범 사례에 맞춰 자동으로 설정합니다.
